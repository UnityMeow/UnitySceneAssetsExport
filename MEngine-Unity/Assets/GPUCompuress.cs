...
public unsafe class GPUCompress : System.IDisposable
{
    ...
    public void Compress(Texture sourceTex, int width, int height, int mip, CommandBuffer cbuffer, bool isHDR)
    {
        ...
        while (num_blocks > 0)
        {
            ...
            cbuffer.SetComputeIntParam(cs, g_tex_width, width);
    ...
    public int GetData(uint4[] values)
    {
        ulong xblocks = (ulong)max(1, (width + 3) >> 2);
        ulong yblocks = (ulong)max(1, (height + 3) >> 2);
